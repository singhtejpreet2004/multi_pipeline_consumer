...
def load_ee_camera_configs():
    ...
    try:
        ...
        logging.critical("[EE Config] Camera config file not found: %s", EE_CAMERA_CFG)
        raise
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            if deltas[dominant_tp] == 0:
                log.warning(
                    "No live traffic detected. Falling back to highest absolute offset."
                )
...
def init_nvml():
    ...
    try:
        ...
        logging.warning(
            ...
            "gpu_utilization column will be empty.",
            exc,
        )
        return None
...
def process_feed(cam_cfg: dict):
    ...
    while consumer is None:
        try:
            ...
            log.info(
                "Warm-up poll complete for topic %s — entering frame consumption loop.",
                topic,
            )
        ...
        try:
            ...
            if highwater is not None:
                ...
                if lag_frames > MAX_LAG_FRAMES:
                    ...
                    log.warning(
                        "Lag Spike: %d frames behind. Teleporting to real-time.",
                        lag_frames,
                    )
            ...
            if SAVE_VIDEO:
                ...
                if (
                    ...
                    and frames_in_current_file >= MAX_FRAMES_PER_AVI
                ):
                    video_writer.release()
                    video_writer = None
                    log.info(
                        "Rotated video to part %d for %s", file_part_number + 1, topic
                    )
                    file_part_number += 1
                    frames_in_current_file = 0
            ...
            if frames_since_commit >= COMMIT_EVERY_N_FRAMES:
                try:
                    ...
                    frames_since_commit = 0
                    log.debug(
                        "Committed offset — topic: %s | partition: %d | frame: %d",
                        topic,
                        dominant_partition,
                        frame_count,
                    )
